    ...
    public function createView($referenceType, $constraints, $viewID) {
        ...
        foreach ($constraints as $table => $constraint) {
            ...
            if (count($constraint) !== 2)
                throw Exception("Invalid VIEW CONSTRAINT");
...
class RedBean_QueryWriter_MySQL extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter {
    ...
    public function sqlStateIn($state, $list) {
        $sqlState = "0";
        if ($state == "42S02")
            $sqlState = RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE;
        if ($state == "42S22")
            $sqlState = RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN;
        if ($state == "23000")
            $sqlState = RedBean_QueryWriter::C_SQLSTATE_INTEGRITY_CONSTRAINT_VIOLATION;
...
where table_schema = 'public'");
    ...
    public function checkChanges($type, $id, $logid) {
        ...
        $newid = $this->insertRecord("__log", array("action", "tbl", "itemid"), array(array(2, $type, $id)));
...
class RedBean_OODB extends RedBean_Observable {
    ...
    public function trash(RedBean_OODBBean $bean) {
        ...
        if (!$this->isFrozen)
            $this->check($bean);
        try {
            $this->writer->selectRecord($bean->getMeta("type"), array($idfield => array($bean->$idfield)), null, true);
        } catch (RedBean_Exception_SQL $e) {
            if (!$this->writer->sqlStateIn($e->getSQLState(), array(
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE)
...
class RedBean_AssociationManager extends RedBean_Observable {
    ...
    public function related(RedBean_OODBBean $bean, $type, $getLinks=false, $sql=false) {
        ...
        if ($type == $bean->getMeta("type")) {
            ...
            $cross = 0;
        if (!$getLinks)
            $targetproperty = $type . "_id"; else
            $targetproperty = "id";
        $property = $bean->getMeta("type") . "_id";
        try {
            $sqlFetchKeys = $this->writer->selectRecord(
                    $table, array($property => array($bean->$idfield)), $sql, false
            );
            $sqlResult = array();
            ...
            if ($cross) {
                $sqlFetchKeys2 = $this->writer->selectRecord(
                        $table, array($targetproperty => array($bean->$idfield)), $sql, false
                );
...
class R {
    ...
    public static function findOrDispense($type, $sql, $values) {
        ...
        if (count($foundBeans) == 0)
            return array(self::dispense($type)); else
            return $foundBeans;
    }
    ...
    public static function untag($bean, $tagList) {
        if ($tagList !== false && !is_array($tagList))
            $tags = explode(",", (string) $tagList); else
            $tags = $tagList;
        foreach ($tags as $tag) {
    ...
    public static function view($viewID, $types) {
        ...
        $types = explode(",", $types);
        if (count($types) < 2)
            throw new RedBean_Exception_Security("Creating useless view for just one type? Provide at least two types!");
...
class SMFWriter extends RedBean_QueryWriter_MySQL {
