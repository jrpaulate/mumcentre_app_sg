...
class Home extends CI_Controller {
    ...
    function index() {
        ...
        $mini_ad1 = $this->_generateTags(41);
        $mini_ad2 = $this->_generateTags(42);
        $mini_ad3 = $this->_generateTags(43);

        $featban = $this->_generateTags(27);
        ...
        $this->bucket->set_data('mini_ad1', $mini_ad1);
        $this->bucket->set_data('mini_ad2', $mini_ad2);
        $this->bucket->set_data('mini_ad3', $mini_ad3);

        $this->bucket->set_data('featban', $featban);        
...
//	$cookie = (($_COOKIE['the_poll_cookie'] != '') ? $_COOKIE['the_poll_cookie'] : 'false');
    ...
    function load_hotbox(&$hotboxcontentarray, &$hotboximgarray, &$hotboxtitlearray) {
        ...
        foreach ($hotbox as $entry) {
            ...
            $contentstring .= "<p class=\"feature_art\"> <span class=\"featured_winner\">" . $hotboxentry['title'] . "</span></p>";
            $contentstring .= "<div class=\"body_art\">" . $hotboxentry['summary'] . "</div><br/>";
            $contentstring .= "<a class=\"blue\" style=\"padding-left: 9px;\" href=\"" . $hotboxentry['url'] . "\">READ MORE</a>";
...
";
	...
	SELECT 'events' as type, a.id, a.provider_id, a.title, a.summary, DATE_FORMAT(a.start_date, '%D %b') as display_date, a.start_date as sort_date, a.seo_url
            ...
            INNER JOIN provider_profile as e ON a.provider_id = e.id
            WHERE a.status = 1
            AND e.status = 1
	...
	SELECT 'programs' as type, c.id, c.provider_id, c.title, c.summary, DATE_FORMAT(c.publish_date, '%D %b') as display_date, c.publish_date as sort_date, c.seo_url
            ...
            INNER JOIN provider_profile as f ON c.provider_id = f.id
            WHERE c.status = 1
            AND f.status = 1
    ...
    private function _upcoming_events() {
        ...
        $sql = "SELECT 'events' as type,a.id, a.provider_id, a.title, a.summary, DATE_FORMAT(a.start_date, '%D %b') as display_date, a.start_date as sort_date, a.seo_url
            ...
            INNER JOIN provider_country as b
            ON b.provider_id = a.provider_id
            INNER JOIN provider_profile as d ON a.provider_id = d.id
            WHERE a.status = 1
            AND d.status = 1
            AND b.country_id = " . $this->config->item('country_id') . "
	AND a.start_date > NOW()
    ...
    private function _reviews_week() {
        ...
        $sql = "SELECT a.title as review_title, a.summary as review_summary, a.provider_id,
	  	  a.image_filepath as review_image, a.id, a.seo_url, b.name as age_group_name
		FROM provider_review AS a
    	INNER JOIN age_group as b ON a.age_group_id = b.id
        INNER JOIN provider_profile as d ON a.provider_id = d.id
	...
	$sql = "SELECT 
        ...
        LEFT JOIN pow_entry as d 
          ON d.id = e.pow_entry_id
        LEFT JOIN pow_contest as b 
          ON b.id  = e.pow_contest_id
        LEFT JOIN pow_contest_category as a 
          ON a.id = d.pow_category_id
        LEFT JOIN user as c 
          ON c.id = d.user_id
        LEFT JOIN pow_contest_country as f 
          ON f.contest_id = e.pow_contest_id 
        WHERE f.country_id = ".$this->config->item('country_id')."
        AND e.concluded_date = (SELECT MAX(concluded_date) FROM pow_result)
...
//    }
    ...
    private function _forum_list() {
        ...
        $sql = "SELECT msg.id_msg, msg.id_topic, msg.id_board, msg.subject, msg.poster_name, msg.body, DATE_FORMAT(FROM_UNIXTIME(msg.poster_time), '%e-%d-%y %r') as 
	time_posted, a.num_replies, a.num_views
	FROM $smfdbname.smf_messages msg
        INNER JOIN $smfdbname.smf_topics AS a
        ON msg.id_topic = a.id_topic
	INNER JOIN $smfdbname.smf_boards sb
        ON sb.id_board = msg.id_board
