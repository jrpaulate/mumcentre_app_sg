...
class Baby extends CI_Controller {
    ...
    function index(){
        ...
        $this->bucket->set_data('age_group', 2);
        $this->bucket->set_data('age_group_title', 'Baby');
        $this->bucket->set_data('age_group_link', 'baby');
        ...
        $this->bucket->set_data('title', 'Mumcentre - Baby articles');
        $this->bucket->set_data('forum_group', 129);
        $this->bucket->set_data('og_image', '');
        $this->bucket->set_data('og_title', 'Mumcentre - Baby articles');
        $this->bucket->set_data('og_type', 'article');
        $this->bucket->set_data('og_url', $base."baby");
    ...
    function read($url_title){
        ...
        $this->bucket->set_data('age_group', "Baby");
        ...
        $this->bucket->set_data('og_type', 'article');
        $this->bucket->set_data('og_url', $base."baby/".$url_title);
        $this->bucket->render_layout();
    }
    ...
    function all(){
        ...
        $this->bucket->set_data('og_title', 'Mumcentre - Baby articles');
        $this->bucket->set_data('og_type', 'article');
        $this->bucket->set_data('og_url', $base."baby/all");
        
        $this->bucket->render_layout();
    }
    
    private function _latest_article() {
    $article = R::getAll("SELECT a.title as article_title, a.summary as article_summary, a.image_filepath as article_image, a.id
    FROM article AS a
  ...
  private function _featured_articles() {
    $articles = R::getAll("SELECT a.title as article_title, a.summary as article_summary, a.image_filepath as article_image, a.id
    FROM article AS a
    WHERE a.age_group_id = 2
...
//    }
    foreach ($articles as &$article) {
        $article['article_link'] = $this->_getArticleLink($article['title']);
        $article['article_summary'] = $this->_trimArticles($article['summary']);
    }
...
//      return $dashed_summary;
    ...
    function baby_data() {
    echo '{"baby_data": [{'.$this->_latest_article().','.$this->_featured_articles().'}]}';
  }
