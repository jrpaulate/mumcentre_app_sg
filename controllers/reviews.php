...
class Reviews extends CI_Controller {
    ...
    function read($review_slug){
        $review = $this->_find_id($review_slug);
        
        $base = base_url();
        $title = $review['title'];
        $image = $review['image_filepath'];
        $review['summary'] = strip_tags($review['summary']);
        $this->load->library('bucket');
        $this->bucket->set_layout_id('mumcentre/athan');
        $this->bucket->set_content_id('mumcentre/reviews');
...
//        $this->bucket->set_data('age_group', "Pregnancy");
        $this->bucket->set_data('review_title', $title);
        $this->bucket->set_data('review_id',$review['id']);
        $this->bucket->set_data('age_group',$review['name']);
        ...
        $this->bucket->set_data('description',$review['summary']);
        $this->bucket->set_data('title', 'Mumcentre | Reviews - '.$title);
        $this->bucket->set_data('og_image', $base."uploaded/provider/review/image/".$image);
        $this->bucket->set_data('og_title', 'Mumcentre - '.$title);
        $this->bucket->set_data('og_type', 'article');
        $this->bucket->set_data('og_url', $base."events/".$review_slug);
        $this->bucket->render_layout();
    }
    
    private function _find_id($review_slug){
       $review_details = R::getRow("SELECT a.id, a.provider_id, a.title, a.summary, a.image_filepath, b.name
            FROM provider_review as a
            INNER JOIN age_group as b ON a.age_group_id = b.id
            WHERE a.seo_url = '".$review_slug."'"
          );
    ...
    function all_reviews_list() {
        echo '{"reviews": [{' . $this->_all_reviews_list() . '}]}';
    }
    
    
    private function _all_reviews_list() {
...
//    }
    ...
    return '"review":' . json_encode($reviews);
  }
