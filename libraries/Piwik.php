...
class Piwik
{
     ...
     * @return  array
     */
    public function page_urls($period = 'day', $cnt = 10)
    {
        $url = $this->piwik_url.'/index.php?module=API&method=Actions.getPageUrls&idSite='.$this->site_id.'&period='.$period.'&date=last'.$cnt.'&format=JSON&token_auth='.$this->token;
        return $this->_get_decoded($url);
    }
    

    public function url_stats($url, $period = 'day', $cnt = 10)
    {
        $url = $this->piwik_url.'/index.php?module=API&method=Actions.getPageUrl&pageUrl='.$url.'&idSite='.$this->site_id.'&period='.$period.'&date=last'.$cnt.'&format=JSON&token_auth='.$this->token;
        return $this->_get_decoded($url);
    }
    
    public function url_stats_range($url,$date_from,$date_to)
    {
        //$url = $this->piwik_url.'/index.php?module=API&method=Actions.getPageUrl&pageUrl='.$url.'&idSite='.$this->site_id.'&period=day&date='.$date_from.','.$date_to.'&format=JSON&token_auth='.$this->token;
		$url = $this->piwik_url.'/index.php?module=API&method=Actions.getPageUrl&pageUrl='.$url.'&idSite=2&period=day&date='.$date_from.','.$date_to.'&format=JSON&token_auth='.$this->token;
        return $this->_get_decoded($url);
    }    
