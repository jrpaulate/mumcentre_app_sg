  ...
  <form id="frmSearch" name="frmMain">
          ...
          <ul>
                        ...
                        <li><div id="age-group-listing"> </div> </li>
                         ...
                         <li><label for="status">Type:</label><select name="type" id="type" onchange="">    
                        ...
                        <input type="text" name="filter_date"  id="search_date"  value="<?php echo set_value('search_date'); ?>"/> </li>
                         ...
                         <li><a id="search_button" href="javascript:;;"  onclick="javascript:Search(); "><img src="assets/img/system/go-btn.gif" width="30px" height="35px"  /></a></li>
...
<script type="text/html" id="data_listing_tpl">
   ...
   <table width="100%" cellpadding="5">
    ...
    {{#article}}
        <tr>
            <td>Topic : </td>
            <td><input id="thread_topic" name="thread_topic" value="{{article_title}}" style="width:100%;border:1px solid #abadb3;line-height:18px;"  /></td>
        </tr>
        

   <tr>
            <td>Message : </td>
            <td><textarea id="thread_msg" name="thread_msg" value="{{article_summary}}" style="width:100%;resize:none;margin:0px;padding:0px;border:1px solid #abadb3;" rows="3">{{{article_summary}}}</textarea></td>
        </tr>
...
<script type="text/html" id="hotbox_listing_tpl">
   ...
   <table width="100%" cellpadding="5">
    ...
    {{#article}}
        ...
        <tr> 
        <td>Article Image</td>
        <td><img id="avatar" value="uploaded/article/image/{{article_image}}" src="uploaded/article/image/{{article_image}}" style="width: 250px; height: 150px;"/></td>
                    
        </tr>
    
        <tr>
            <td>Title : </td>
            <td><input id="title" name="title" value="{{article_title}}" style="width:100%;border:1px solid #abadb3;line-height:18px;"  /></td>
        </tr>
        <tr>
            <td>Summary : </td>
            <td><textarea id="summary" name="summary" value="{{{article_summary}}}" style="width:100%;resize:none;margin:0px;padding:0px;border:1px solid #abadb3;" rows="3">{{article_body}}</textarea></td>
        </tr>
        
        
         <input type="hidden" id="seo_url" value="{{age_group_name}}/{{seo_link}}"/>  
         <input type="hidden" id="age_group" value="{{age_group_name}}" />
...
<script type="text/html" id="article_all_listing_tpl">
    <table id="myTable" class="datatable" width="100%" cellpadding="0" cellspacing="0" border="0" >
        	...
        	<tr>
        	  ...
        	  <td width="5%"><img src="uploaded/article/image/{{article_image}}" width="65" height="65" /></td>
                  <td width="6%">{{article_type}}</td>
                  <td width="7%">{{age_group_name}}</td>
        	  <td width="20%">{{article_title}}</td>
        	  
                  <td width="5%">{{status}}</td>
        	  <td width="17%"></br><a class="actionbtn" onclick="javascript:preview({{id}})" href="javascript:;;" id ="">Preview</a> | <a class="actionbtn" href="edit_article/read/{{id}}" id ="">Edit</a></br> </br><a class="actionbtn" onclick="javascript:publish({{id}})" href="javascript:;;" >Publish</a> | <a class="actionbtn" onclick="javascript:unpublish({{id}})" href="javascript:;;" >Unpublish </a> </br> </br> <a class="actionbtn" onclick="javascript:open_modal({{id}})" href="javascript:;;" id ="" >Publish to Forums </a>  |  <a class="actionbtn" onclick="javascript:open_hotbox({{id}})" href="javascript:;;" id ="" >Add to Hotbox </a></br> </br></br> <a class="actionbtn" onclick="javascript:remove({{id}})" href="javascript:;;" id ="" >Delete This Content </a></br></br></td>
...
<script type="text/html" id="article_search_listing_tpl">
    <table id="myTable" class="datatable" width="100%" cellpadding="0" cellspacing="0" border="0" >
            {{#article_list}}
            {{#article}}
        	<tr>
...
<script type="text/html" id="article_tpl">
    ...
    {{#article_data}}
       ...
       <div id="Title" style=" font-family: Arial, Helvetica, sans-serif; font-size:18px; color: #3399cc; padding-left: 20px; margin-top: 12px;">
         <tr>
         <td>{{article_title}}</td>
        ...
        <div id="articlecontain" style="float:left; padding-left: 18px; width: 350px;" align="justify">
        {{summary}}
        </div><!--end of Content-->
</div>
...
<script type="text/javascript">
    $(document).ready(function(){
      ...
      $.get("cms/all_list", function(response) {
            var data = JSON.parse(response);
            var template = ich.article_all_listing_tpl(data);
        ...
        $.get("cms/country_list", function(response) {
         ...
         $('#frmSearch').submit(function(e) {
            ...
            $.get("cms/search_article/",{keyword:keyword},function(response) {
                var data = JSON.parse(response);
                var template = ich.article_search_listing_tpl(data);
                $('#article-listing').html('');
                $('#article-listing').append(template); 
                return false;    });
            e.preventDefault();
...
<script>
 ...
 function open_modal(article_id){
       ...
       $.get("cms/article_data/"+ article_id, function(response) {
            var data = JSON.parse(response);
...
<script>
 ...
 function open_hotbox(article_id){
       ...
       $.get("cms/article_data/"+ article_id, function(response) {
            ...
            var html_data = $('#summary').text();
                $('#summary').text('');
                $('#summary').html(html_data);
        });
...
<script>
 ...
 function publish (article_id) {
           ...
           $.post("cms/force_publish/"+ article_id) 
...
<script>
    ...
    $.get("cms/article_list/"+ country_id+"/"+ age_group_id+"/" + type+"/" +status+"/" +date, function(response) {
            var data = JSON.parse(response);
...
<script>
 ...
 function remove (article_id) {
           ...
           //$.post("cms/force_delete/"+article_id+"/"+title) 
		   ...
		   $.post("cms/force_delete/"+article_id, {
                    
                },
                function(data){
                    alert(data);
					if (data=="Article Removed"){
					//window.location = 'cms/article';
                    location.href='cms/article';                   
					}
                });
