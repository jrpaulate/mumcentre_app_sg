        ...
        <div id="PSPP-head-cont">
            <div id="PSPP-head-Lkcont">
                ...
                <div id="menu_curriculum" class="PSPP-Hlk-cont">
                    <a id="active" href="ps_providers/curriculums/<?php echo $provider_id; ?>" class="pspplnk">
                        <div class="PSPP-HlK-box">
...
<script type="text/html" id="curriculum_list_tpl">
    {{#curriculum_list}}
    {{#curriculums}}
    <li id="curriculum_{{id}}">
        <div class="PSPE-Content-img">
            <img src="uploaded/provider/curriculum/image/{{curriculum_image}}" width="100" height="70" />
        </div>
        <div class="PSPE-Content-info-cont">
            <div class="PSPE-info-header">
                <a href="javascript:;;">{{curriculum_title}}</a>
            </div>
            <div class="PSPE-info-quotation">
                            	{{curriculum_summary}}
            </div>
            <div id="curriculum_{{id}}" class="PSPE-info-link" onclick="javascript:expand({{id}})">
                <a href="javascript:;;">Click here to view complete details of this curriculum.</a>
            </div>
    ...
    <div class="PSPE-CD-Container">
        ...
        <div class="PSPE-CD-info">
            <div class="PSPE-CD-Img">	
                <img src="uploaded/provider/curriculum/image/{{curriculum_image}}" width="300" height="250" />                          </div>
            <h2>{{curriculum_title}}</h2><br/>
            <div id="curriculum_body_{{id}}">{{curriculum_body}}</div>
        </div> <!-- PSPE CD info -->
...
<script type="text/html" id="curriculum_close">
    {{#curriculum_data}}
    {{#curriculum}}
    <div class="PSPE-Content-img">
        <img src="uploaded/provider/curriculum/image/{{curriculum_image}}" width="100" height="70" />
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $.get("ps_providers/curriculum_list/"+provider_id, function(response) {
            var data = JSON.parse(response);
            var template = ich.curriculum_list_tpl(data);
            $('#curriculum-listing').html('');
            $('#curriculum-listing').append(template);
            
            if(part_id > 0) {
                $('html, body').animate({ scrollTop: $("#curriculum_"+part_id).offset().top }, 500);
                expand(part_id);
            }
            
            $('#page_container').pajinate({
                item_container_id : '#curriculum-listing',
    ...
    function shrink(id){
        ...
        $.get("ps_providers/curriculum_data/"+id, function(response) {
            var data = JSON.parse(response);
            var template = ich.curriculum_close(data);
            $('li#curriculum_'+id).html('');
            $('li#curriculum_'+id).append(template);
            $('li#curriculum_'+id).css('height','185');
        });  
    } 
</script>
...
<script type="text/javascript">
    function expand(id){
        $('div#curriculum_'+id).html('');
        $('div#curriculum_'+id).append('<img src="assets/img/system/loading.gif" alt="Loading" id="loading" />');
        ...
        $.get("ps_providers/curriculum_data/"+id, function(response) {
            ...
            $('li#curriculum_'+id).html('');
            $('li#curriculum_'+id).append(template);
            $('li#curriculum_'+id).css('height','auto');
            
            var html_data = $('#curriculum_body_'+id).text();
            $('#curriculum_body_'+id).text('');
            $('#curriculum_body_'+id).html(html_data);
