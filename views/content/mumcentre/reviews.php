...
<div id="body2com">
    <div id="content1" class="clear1">
        <div id="content-main">
            <div id="section-head">
                <ul class="breadcrumb">
                    <li class="active">Reviews<span class="divider"><?= img('system/Header-Arrow.png'); ?></span></li>
                    <li class="active"><?php echo $age_group; ?><span class="divider"><?= img('system/Header-Arrow.png'); ?></span></li>
                    <li class="active"><?php echo $review_title; ?></li>
                </ul>
            ...
            <div id="PSPE-MBcontainer">
                <div id="page_container">
                    ...
                    <ul id="review-listing" class="RCAA-ul"></ul>
                    <div id="RCAA-pagination"></div>
                </div>
            </div>
...
<script type="text/html" id="review_list_tpl">
    {{#reviews}}
            {{#review}}
            <li>
                <div class="PSPE-Content-img">
                    <img width="90" height="75" src="uploaded/provider/review/image/{{image_filepath}}"/>
                </div>
                <div class="PSPE-Content-info-cont">
                    <div class="PSPE-info-header">
                        <a href="reviews/{{seo_url}}">{{title}}</a>
                    </div><br/>
                    <div class="PSPE-info-quotation" style="height: 100px"><br/>
                            	{{{summary}}}
                    </div>
                    <div class="PSPE-info-link">
                        <a href="reviews/{{seo_url}}">Click here for complete details on this this review.</a>
                    </div>
            ...
            {{^review}}
            <li>
                <span>No reviews available for the chosen time period.</span>
            </li> 
            {{/review}}
        {{/reviews}}
</script>
    ...
    <div class="PSPE-CD-Container">
        ...
        <div class="PSPE-CD-info">
            <div class="PSPE-CD-Img">
                ...
                <img src="uploaded/provider/review/image/{{review_image}}" width="300" height="250" />                          </div>
            <h2>{{review_title}}</h2><br/>
            <div id="review_body">{{{review_body}}}</div>
...
<script type="text/javascript">
        $(document).ready(function(){
            ...
            $.get("reviews/all_reviews_list", function(response) {
                var data = JSON.parse(response);
                var template = ich.review_list_tpl(data);
                $('#review-listing').html('');
            ...
            $.get("ps_providers/review_data/"+review_id, function(response) {
                var data = JSON.parse(response);
                var template = ich.review_tpl(data);
                $('#review_itself').html('');
                $('#review_itself').append(template);
            
                var html_data = $('#review_body').text();
                $('#review_body').text('');
                $('#review_body').html(html_data);
                return false;
            });
            ...
            $('#event_prog_time').click(function(e){
                ...
                if (from.length == 0){
                    ...
                    $.get("ps_providers/reviews_time/"+from+"/"+to, function(response) {
                        var data = JSON.parse(response);
                        var template = ich.review_list_tpl(data);
                        $('#review-listing').html('');
                        $('#review-listing').append(template);
