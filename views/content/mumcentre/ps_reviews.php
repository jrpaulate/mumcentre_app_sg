        ...
        <div id="PSPP-head-cont">
            <div id="PSPP-head-Lkcont">
                ...
                <div id="menu_review" class="PSPP-Hlk-cont">
                    <a id="active" href="ps_providers/reviews/<?php echo $provider_id; ?>" class="pspplnk">
                        <div class="PSPP-HlK-box">
                            <div class="PSPP-HlK-text">
                                Reviews
                            </div>
        ...
        {{#review_list}}
                    ...
                    <li id="review_{{id}}">
                        <div class="PSPE-Content-img">
                            <img src="uploaded/provider/review/image/{{review_image}}" width="100" height="70" />
                        </div>
                        <div class="PSPE-Content-info-cont">
                            <div class="PSPE-info-header">
                                <a href="javascript:;;" id="title">{{review_title}}</a>
                            </div>
                            <div class="PSPE-info-quotation">
                                <p>{{review_summary}}</p>
                            </div>
                            <div  id="review_{{id}}" class="PSPE-info-link" onclick="javascript:expand({{id}})">
                                <a href="javascript:;;" id="clicker">Click here for complete details of this review.</a>
                            </div>
...
<script type="text/html" id="review_tpl">
    ...
    <div class="PSPE-CD-Container">
        ...
        <div class="PSPE-CD-topcont">
            <div id="share" class="PSPE-CD-socialT">
                <div id="toolbox_{{id}}" name="toolbox_{{id}}" class="addthis_default_style" addthis:url="<?php echo base_url();?>ps_providers/reviews/<?php echo $provider_id;?>/{{id}}"></div>
            </div>
        ...
        <div class="PSPE-CD-info">
            <div class="PSPE-CD-Img">	
                <img id="review_image" src="uploaded/provider/review/image/{{review_image}}" width="300" height="250" />                          </div>
            <h2>{{review_title}}</h2><br/>
            <div id="review_body_{{id}}" class="provider_expanded_content">{{review_body}}</div>
        </div> <!-- PSPE CD info -->
        <div class="PSPE-CD-socialB">
            <div id="toolbox2_{{id}}" name="toolbox2_{{id}}" class="addthis_default_style" addthis:url="<?php echo base_url();?>ps_providers/reviews/<?php echo $provider_id;?>/{{id}}"></div>
        </div>
        <!--                    	<div class="PSPE-dot-bar"></div>-->
        {{/review}}
...
<script type="text/html" id="review_close">
        {{#review_data}}
                    {{#review}}
                        <div class="PSPE-Content-img">
                            <img src="uploaded/provider/review/image/{{review_image}}" width="100" height="70" />
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $.get("ps_providers/review_list/"+provider_id, function(response) {
            ...
            if(part_id > 0) {
            $('html, body').animate({ scrollTop: $("#review_"+part_id).offset().top }, 500);
            expand(part_id);
            }
            
            $('#page_container').pajinate({
            item_container_id : '#review-listing',
    ...
    function shrink(id){
         ...
         $.get("ps_providers/review_data/"+id, function(response) {
                var data = JSON.parse(response);
                var template = ich.review_close(data);
                $('li#review_'+id).html('');
                $('li#review_'+id).append(template);
                $('li#review_'+id).css('height','185');
         });  
     } 
</script>
...
<script type="text/javascript">
    function expand(id){
        $('div#review_'+id).html('');
        $('div#review_'+id).append('<img src="assets/img/system/loading.gif" alt="Loading" id="loading" />');
        ...
        $.get("ps_providers/review_data/"+id, function(response) {
            ...
            $('li#review_'+id).html('');
            $('li#review_'+id).append(template);
            $('li#review_'+id).css('height','auto');
            var html_data = $('#review_body_'+id).text();
            var review_image = $('#review_image_'+id).attr('src');
            $('#review_body_'+id).text('');
            $('#review_body_'+id).html(html_data);
