...
abstract class OpenInviter_Base {
  ...
  protected function init($cookie_file = false) {
    if ($cookie_file)
      ...
      $this -> cookie = $this -> settings['cookie_path'] . '/' . $this -> service_user . "_" . time();
  ...
  protected function get($url, $follow = false, $header = false, $quiet = true, $referer = false, $headers = array()) {
    if ($this -> settings['transport'] == 'curl') {
      ...
      if ((strlen($buffer) == 0) or ($return_var != 0))
        return (false);
      else
        return $buffer;
    }
  ...
  protected function updateDebugBuffer($step, $url, $method, $response = true, $elements = false) {
    $this -> debug_buffer[$step] = array('url' => $url, 'method' => $method);
    if ($elements)
