 ...
 */
$_pluginInfo = array('name' => 'GMail', 'version' => '1.3.9', 'description' => "Get the contacts from a GMail account", 'base_version' => '1.6.0', 'type' => 'email', 'check_url' => 'http://google.com');
/**
 * GMail Plugin
 *
...
class gmail extends OpenInviter_Base {
  ...
  public $debug_array = array('login_post' => 'Auth=', 'contact_xml' => 'entry');
  ...
  public function getMyContacts() {
    ...
    foreach ($data as $node) {
      ...
      foreach ($entry_nodes as $child) {
        ...
        if (!empty($name))
          $contacts[$child -> getAttribute('address')] = $name;
        else
          $contacts[$child -> getAttribute('address')] = $child -> getAttribute('address');
      }
    }
    return $contacts;
  }
