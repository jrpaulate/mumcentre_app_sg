<?php
$_pluginInfo = array('name' => 'Yahoo!', 'version' => '1.3.7', 'description' => "Get the contacts from a Yahoo! account", 'base_version' => '1.6.0', 'type' => 'email', 'check_url' => 'http://mail.yahoo.com');
/**
 * Yahoo! Plugin
 *
...
class yahoo extends OpenInviter_Base {
  ...
  public $debug_array = array('initial_get' => 'form: login information', 'contacts_page' => 'import_export', 'export_post' => 'crumb2', 'contacts_file' => '","');
  ...
  public function login($user, $pass) {
    ...
    $post_elements = $this -> getHiddenElements($res, 'login', $user, 'passwd', $pass);
  ...
  public function getMyContacts() {
    ...
    if ($this -> checkResponse("contacts_page", $res)) {
      ...
      foreach ($data as $val) {
        if (strstr($val -> getAttribute('href'), "import_export")) {$url_contacts = "http://address.mail.yahoo.com/" . str_replace("&amp;", "&", $val -> getAttribute('href'));
          break;
        }
      }
    ...
    if ($this -> checkResponse('export_post', $res)) {
      ...
      libxml_use_internal_errors(false);
      $xpath = new DOMXPath($doc);
      $query = "//input[@id='crumb2']";
      $data = $xpath -> query($query);
      foreach ($data as $val)
        $post_elements = array("VPC" => "import_export", $val -> getAttribute('name') => $val -> getAttribute('value'), "submit[action_export_yahoo]" => "Export+Now");
    ...
    if ($this -> checkResponse("contacts_file", $res)) {
      ...
      foreach ($temp as $values) {
        ...
        if (!empty($values['16'])) {$contacts[$values['16']] = (empty($name) ? $values['16'] : $name);
          $ok_id_mes = false;
        }
        if (!empty($values['17'])) {$contacts[$values['17']] = (empty($name) ? $values['17'] : $name);
          $ok_id_mes = false;
        }
        if (!empty($values['4'])) {$contacts[$values['4']] = (empty($name) ? $values['4'] : $name);
          $ok_id_mes = false;
        }
        if ($ok_id_mess) {
          if (!empty($values['7'])) { $mess_id = (!$this -> isEmail($values['7']) ? "{$values['7']}@yahoo.com" : $values['7']);
            $contacts[$mess_id] = (empty($name) ? $mess_id : $name);
          } elseif (!empty($values['52']))
            $contacts[$values['52']] = (empty($name) ? $values['52'] : $name);
        }
      }
